...
function handleDragEnter(e){
  if(dragSrcEl!=null) this.classList.add('over');// this / e.target is the current hover target.
}
...
function handleDrop(e){
	...
	//Removing over of the all elements
	var t=document.getElementsByClassName('over');
	for(var i=0;i<t.length;i++) t[i].classList.remove('over');
	
	if(dragSrcEl!=null){//falta seleccion multiple
		var temp=new Array();
		if(e.dataTransfer.effectAllowed=='copy'){
			temp.push(document.importNode(dragSrcEl,true));
			addEvents(temp[0]);
		}
